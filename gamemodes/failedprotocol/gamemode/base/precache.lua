...
function BuildPrecacheList()
	...
	for _, vest in pairs( REGISTERED_ARMOR ) do
		table.insert( precache_list, vest.floor_model )
		table.insert( precache_list, vest.model )
	end
...
if SERVER then
	function PrecacheAll()
		if FP_PRECACHE_DONE or precache_len == 0 then return end

		util.TimerCycle()
		print( "Precaching "..precache_len.." models..." )
		hook.Add( "Tick", "FPPrecacheModels", function()
			for i = 1, 10 do
			...
			if current_index == -1 then
				...
				hook.Remove( "Tick", "FPPrecacheModels" )

				FP_PRECACHE_DONE = true
			else
	...
	hook.Add( "PostGamemodeLoaded", "FPPrecacheModels", function()
		timer.Simple( 7.77, function()
			BuildPrecacheList()
			PrecacheAll()
		end )
	end )
