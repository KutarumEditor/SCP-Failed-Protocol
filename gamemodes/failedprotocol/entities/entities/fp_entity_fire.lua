-- CUSTOM FIRE SCRIPT IS MADE BY DANX (https://github.com/danx91/SCP-LC/blob/master/gamemodes/scplc/entities/entities/slc_entity_fire.lua)
...
function ENT:Initialize()
	...
	hook.Run( "FPOnEntityIgnited", self:GetParent(), self )
end
...
function ENT:Think()
	...
	if IsValid( parent ) and self.HurtParent and ( self.HurtOwner or parent != owner ) then
		...
		if hook.Run( "FPFireOnBurnPlayer", self, parent, dmg ) != true then
			parent:TakeDamageInfo( dmg )
		end
	end

	if self.Radius <= 0 or self.Power < 20 then return end
	for i, v in ipairs( ents.FindInSphere( self:GetPos() + self.Offset, self.Radius ) ) do
	--for i, v in ipairs( ents.FindInSphere( self:GetPos() + self.Offset, self.Radius, -48, 48, nil, MASK_SOLID_BRUSHONLY, player.GetAll() ) ) do
		if !v:IsPlayer() or v == parent or v:FPTeam() == TEAM_SPEC or !self.HurtOwner and v == owner then continue end
		if IsValid( owner ) and owner:IsPlayer() and owner:FPTeam() == TEAM_SCP and v:FPTeam() == TEAM_SCP then continue end
		...
		if final_dmg > 0 then
			...
			if hook.Run( "FPFireOnBurnPlayer", self, v, dmg ) != true then
				v:TakeDamageInfo( dmg )
			end
		end

		if self.ShouldSpread then
			if FPRandom( 100 ) <= self.Power then
